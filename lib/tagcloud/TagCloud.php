...
class TagCloud
{
  ...
  public function formatTag($string)
  {
    ...
    return strip_tags($string);//preg_replace('/[^\w ]/u', '', strip_tags($string));
  }
  ...
  public function addTag($tagAttributes = array())
  {
    ...
    $tagAttributes['original-tag'] = $tagAttributes['tag'];
  ...
  public function htmlizeTag($arrayInfo, $sizeRange)
  {
    ...
    if (isset($htmlizeTagFunction) &&
      ...
      return "<span class='tag size{$sizeRange}'> &nbsp; <a href='catalog?nerd_entity={$arrayInfo['original-tag']}'>{$arrayInfo['tag']}</a><sup style='font-size: 14px; color:#aaa'>({$arrayInfo['size']})</sup> &nbsp; </span>";
    }
  }
